...
def fund_money_fund_info_em(symbol: str = "000009") -> pd.DataFrame:
    ...
    def fetch_page(page: int):
        ...
        page_params.update({"pageIndex": page})
        page_r = requests.get(url, params=page_params, headers=headers)
        page_json = page_r.json()
        return page, pd.DataFrame(page_json["Data"]["LSJZList"])

    with ThreadPoolExecutor(max_workers=min(8, total_page)) as executor:
        futures = {executor.submit(fetch_page, page): page for page in page_list}
        for future in tqdm(as_completed(futures), total=len(page_list), leave=False):
            page, temp_df = future.result()
            page_dfs[page] = temp_df

    big_list = [page_dfs[page] for page in page_list]
...
def fund_etf_fund_info_em(
    ...
    big_df = pd.concat([page_dfs[page] for page in page_list])
    big_df.columns = [
        "净值日期",
        "单位净值",
        "累计净值",
