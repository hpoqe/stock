...
def fetch_paginated_data(url: str, base_params: Dict, timeout: int = 15):
    ...
    # 获取剩余页面数据
    remaining_pages = list(range(2, total_page + 1))
    if remaining_pages:
        tqdm = get_tqdm()

        def fetch_page(page: int) -> pd.DataFrame:
            current_params = base_params.copy()
            current_params.update({"pn": page})
            time.sleep(random.uniform(0.5, 1.5))
            r = request_with_retry(url, params=current_params, timeout=timeout)
            page_json = r.json()
            return pd.DataFrame(page_json["data"]["diff"])

        with ThreadPoolExecutor(max_workers=min(8, len(remaining_pages))) as executor:
            future_to_page = {
                executor.submit(fetch_page, page): page for page in remaining_pages
            }
            for future in tqdm(
                as_completed(future_to_page), total=len(remaining_pages), leave=False
            ):
                inner_temp_df = future.result()
