...
def stock_individual_fund_flow_rank(indicator: str = "5日") -> pd.DataFrame:
    ...
    def fetch_page(page: int):
        ...
        page_params.update({"pn": page})
        page_r = requests.get(url, params=page_params, timeout=15)
        page_json = page_r.json()
        return page, pd.DataFrame(page_json["data"]["diff"])
    ...
    with ThreadPoolExecutor(max_workers=min(8, total_page)) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(page_list), leave=False):
            page, inner_temp_df = future.result()
            page_dfs[page] = inner_temp_df

    temp_list = [page_dfs[page] for page in page_list]
